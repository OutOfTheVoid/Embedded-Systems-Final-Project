...
#include <stdint.h>
#include "tivaware_tm4c123gh6pm.h"
#include "SSD2119.h"
...
void LCD_GPIOInit(void){
    ...
    gpio_enable_port(kPortB);
                                            // activate port B
    wait++;                                // wait for port activation
    wait++;                                // wait for port activation
                                           // make PB0-7 outputs
                                           // disable alternate functions 
    gpio_setup_digital_output_pin_group(kPortB, PINMASK_ALL, kDriveCurrent_2ma, kPinDrive_PullDown);
                                           // enable digital I/O on PB0-7
    gpio_enable_digital_pin_group(kPortB, PINMASK_ALL);
    
    // activate control pins
                                          // activate port A
    gpio_enable_port(kPortA);
    wait++;                               // wait for port activation
    wait++;                               // wait for port activation
                                          // make PA4-7 outputs
                                          // disable alternate functions 
    gpio_setup_digital_output_pin_group(kPortA, PINMASK_4 | PINMASK_5 | PINMASK_6 | PINMASK_7, kDriveCurrent_2ma, kPinDrive_PullDown);
                                          // enable digital I/O on PA4-7
    gpio_enable_digital_pin_group(kPortA, PINMASK_4 | PINMASK_5 | PINMASK_6 | PINMASK_7);
...
//    TOUCH_YP = 0xFF;
    ...
    gpio_enable_port(kPortA);
    gpio_setup_digital_output_pin_group(kPortA, PINMASK_2 | PINMASK_3, kDriveCurrent_2ma, kPinDrive_PullDown);
    gpio_enable_port(kPortE);
    gpio_setup_digital_output_pin_group(kPortE, PINMASK_4 | PINMASK_5, kDriveCurrent_2ma, kPinDrive_PullDown);
}
